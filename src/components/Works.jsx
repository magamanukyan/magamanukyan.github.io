...
const ProjectCard = ({index, name, description, tags, image, source_code_link, source_link})=>{
  return (
    <motion.div onClick={()=>window.open( source_link, '_blank')} variants={fadeIn('up', 'spring', index*0.5, 0.75)}>
      <Tilt
        options={{
          max:45,
          scale:1,
            ...
            <div className='absolute inset-0 flex justify-end m-3 card-img_hover'>
              ...
              <div onClick={()=>window.open( source_link, '_blank')}
                    className='black-gradient ml-2 w-10 h-10 rounded-full flex justify-center items-center cursor-pointer'>
                        <img src={project} alt="github" className='w-1/2 h-1/2 object-contain' />
              </div>
            </div>
          </div>
          <div className='mt-5'>
